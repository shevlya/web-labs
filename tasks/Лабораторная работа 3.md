...
-- Добавляем внешний ключ в таблицу task на user_account
ALTER TABLE task ADD CONSTRAINT fk_task_created_by FOREIGN KEY (created_by) REFERENCES user_account(id);
```
...
-- Пользователи
INSERT INTO user_account (username) VALUES ('admin'), ('user');

-- Связи (админ с ролью ADMIN, пользователь с ролью USER)
...
### UserRepository +
В рамках подготовки к следующей лабораторной работе реализуйте также репозиторий работы с пользователем с одним пользовательским методом: получение пользователя по username. Реализовать через [Query method](https://docs.spring.io/spring-data/commons/reference/repositories/query-methods.html).
